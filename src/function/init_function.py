    ...
    while True:
        try:
            if args_catalog == 'Xmm_DR13':
                ...
                print("-"*50, "\n")
                return valid_path, args_catalog
            elif args_catalog == 'eRASS1':
                print("\n")
                print(f"{colored(args_catalog, 'yellow')} catalog is loading")
                eRASS1_path = os.path.join(catalog_datapath, "eRASS1.fits").replace("\\", "/")
                print("-"*50)
                valid_path = get_valid_file_path(eRASS1_path)
                print("-"*50, "\n")
                return valid_path, args_catalog
            elif args_catalog == "compare_catalog":
                print("\n")
                print("Enter catalog keyword (Xmm_DR13/CSC_2.0/Swift/eRASS1)")
                ...
                while True:
                    if catalog_1 == "Xmm_DR13":
                        ...
                        break
                    elif catalog_1 == "eRASS1":
                        catalog_1_path = os.path.join(catalog_datapath, "eRASS1.fits").replace("\\", "/")
                        break
                    else:
                ...
                while True:
                    if catalog_2 == "Xmm_DR13":
                        ...
                        break
                    elif catalog_2 == "eRASS1":
                        catalog_2_path = os.path.join(catalog_datapath, "eRASS1.fits").replace("\\", "/")
                        break
                    else:
                ...
                raise argparse.ArgumentError(None, "invalid catalog keyword keyword. retry with Xmm_DR13, CSC_2.0, Swift, eRASS1, compare_catalog or match.")
...
def py_to_xlsx(excel_data_path: str, count_rates: List, object_data: Dict, args: Tuple[str, str], radius: float) -> None:
    ...
    Args:
        ...
        args (Tuple[str, str]): Tuple of additional arguments, typically catalog identifiers 
                                (e.g., 'Xmm_DR13', 'CSC_2.0', 'Swift', 'eRASS1', 'match').
